...
\newline
\newline This project will be arranged as following: we will first look at the number of twitter on Starbucks compared with Dunkin' Donuts and their distribution in the US on a map to see which one is popular as a topic on social media. Then, we want to show what are people talking about Starbucks when mentioning it on Twitter by using word clouds. Further analysis is related to sentiment analysis. We use ordinal logistic regression to see the relationship between location and sentiment to Starbucks (positive, neutral or negative). Shiny app is used for visualization here. Taking diffrent level of influence of each tweets has on other users, we also conduct analysis on the variables that can reflect the influence level of a tweet. We use kernel density estimation to get plots of density of the variables and use bootstrap to estimate their means.
...
The link https://sijiexiang.shinyapps.io/Final_Project/ is a shiny app that provides interactive bar chart to show the number of tweets that fall into different sentiment in one of the four regions.
